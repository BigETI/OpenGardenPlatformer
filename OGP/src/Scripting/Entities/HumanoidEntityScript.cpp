...
void HumanoidEntityScript::OnGameTick(Engine& engine, high_resolution_clock::duration deltaTime) {
	if (IsAlive()) {
		if (shared_ptr<GardenScript> current_garden = GetGarden().lock()) {
			...
			do {
				...
				else if (!current_garden->IsWalkableAt(current_position) && !current_garden->IsClimbableAt(current_position)) {
					is_falling = true;
				}
				Vector2<int> remaining_movement(is_falling ? Vector2<int>(0, -1) : (targetPosition.GetConverted<int>() - current_position.GetConverted<int>()));
				...
				if (is_not_moving) {
					...
					if (is_falling && (targetPosition == (current_position + Vector2<size_t>(static_cast<size_t>(1), static_cast<size_t>(0))))) {
						targetPosition = current_position;
						toBeRenderedAtOffset = Vector2<float>();
					}
				}
